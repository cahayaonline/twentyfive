...
class ProfileMember extends MainPageMembers {
	...
	public function populateData() {
		...
		if ($r['status'])
			$this->menikah->Checked=true;
		else
			$this->lajang->Checked=true;								
		...
		if ($r['tax_status'])
			$this->pajakmenikah->Checked=true;
		else
			$this->pajaklajang->Checked=true;
	...
	public function saveData ($sender,$param) {
		if ($this->IsValid) {							
			...
			$pajakmenikah=$this->pajakmenikah->Checked==true?1:0;
			...
			if ($password =='') {
				$str="UPDATE `members` SET `member_name`='$nama', `identity_num`='$ktp', `gender`='$jk', `birthday`='$tgl', `status`=$status, `patner_name`='$nama_pasangan', `identity_num_patner`='$ktp_pasangan', `address`='$alamat', `city`='$kota', `postal_code`='$kodepos', `country_id`=$negara, `phone_number`='$telp', `email`='$email', `mobile_phone`='$hp', `mobile_phone2`='$hp2', `mother_name`='$nama_ibu',`family_name`='$ahli_waris', `relationship_family`='$hubungan_ahli', `account_number`='$norek', `account_name`='$atasnama', `bank_name`='$namabank',`branch`='$cabang', `bank_location`='$lokasibank', `tax_number`='$npwp', `tax_status`=$pajakmenikah, `responsible_number`='$jumlah_tanggungan', `notes`='$notes' WHERE member_id=$member_id";
			}else {
				$password=$this->Pengguna->createHashPassword($this->txtPassword1->Text);
				$str="UPDATE `members` SET `member_name`='$nama', `identity_num`='$ktp', `gender`='$jk', `birthday`='$tgl', `status`=$status, `patner_name`='$nama_pasangan', `identity_num_patner`='$ktp_pasangan', `address`='$alamat', `city`='$kota', `postal_code`='$kodepos', `country_id`=$negara, `phone_number`='$telp', `email`='$email', `mobile_phone`='$hp', `mobile_phone2`='$hp2', `mother_name`='$nama_ibu',`family_name`='$ahli_waris', `relationship_family`='$hubungan_ahli', `account_number`='$norek', `account_name`='$atasnama', `bank_name`='$namabank',`branch`='$cabang', `bank_location`='$lokasibank', `tax_number`='$npwp', `tax_status`=$pajakmenikah, `responsible_number`='$jumlah_tanggungan', `notes`='$notes',salt='{$password['salt']}', password='{$password['password']}' WHERE member_id=$member_id";
			}		
// 			echo $str;	
			$this->DB->updateRecord($str);
			$this->member->redirect('m.members.ProfileMember',array('member_id'=>$member_id));
		}
	}
}
